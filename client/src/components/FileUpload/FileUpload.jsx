...
const FileUpload = () => {
  ...
  const uploadPostHandler = async () => {
    ...
    files.map((file) => {
      if (
        Object(file).type == "image/jpeg" ||
        Object(file).type == "image/png"
      ) {
    ...
    files.map((file) => {
      if (
        Object(file).type == "video/mp4" ||
        Object(file).type == "video/ogg"
      ) {
  ...
  const uploadHandler = async (e) => {
    if (
      Object(e.target.files[0]).type != "image/jpeg" &&
      Object(e.target.files[0]).type != "image/png" &&
      Object(e.target.files[0]).type != "video/mp4" &&
      Object(e.target.files[0]).type != "video/ogg"
    ) {
      ...
      <div style={{ overflowX: "none", height: "100vh" }}>
        ...
        {show == 0 ? (
          <div className="file-card">
            ...
            <p className="info">
              Upload photos and videos with wonderful title and description.
              <br></br>
              <p style={{ color: "red", fontSize: "14px" }}>
                Only image/jpeg,image/png,video/mp4 or video/ogg files are
                accepted
              </p>
            </p>
          ...
          >
            ...
            <div className={classes.image}>
              {(Object(files[index]).type == "image/jpeg" ||
                Object(files[index]).type == "image/png") && (
                <img
                  src={src[index]}
                  style={{ width: "100%", height: "300px" }}
                ></img>
              )}
              {(Object(files[index]).type == "video/mp4" ||
                Object(files[index]).type == "video/ogg") && (
                <video style={{ width: "100%", height: "300px" }} controls>
                  <source
                    src={src[index]}
                    type={Object(files[index]).type}
                  ></source>
                </video>
              )}
              {index !== 0 && (
