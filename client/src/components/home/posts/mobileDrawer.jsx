...
const MobileDrawer = () => {
  ...
  const leftBar = {
    ...
    marginLeft: "auto",
    zIndex:
      "100000454648748787897489999999999999999999999999999999999999999999999999987867468998099999999999999999999999999999",
    paddingTop: "70px",
    opacity: "1",
    "&::-webkit-scrollbar": {
      width: "5px",
  ...
  useEffect(() => {
    ...
    online = online.filter((user) =>
      auth.user.user.following.includes(user.userId)
    );
    console.log(online, "online");
    console.log(users, "allusers");
    setOnlineFriends(
      users.filter((user) =>
        online.map((user) => user.userId).includes(user._id)
      )
    );
  }, [onlineusers, users]);
      ...
      >
        <Component>
          ...
          >
            <FontAwesomeIcon
              icon={faRss}
              style={{ paddingRight: "20px", width: "20px" }}
            />
          ...
          <Section onClick={handleClick}>
            <Box style={{ position: "relative" }}>
              <FontAwesomeIcon
                ...
                style={{ paddingRight: "20px", width: "20px" }}
              />
              {notifications.notifications.request > 0 && (
                <Typography
                  style={{
                    position: "absolute",
                    backgroundColor: "red",
