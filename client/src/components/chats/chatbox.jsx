  ...
  useEffect(() => {
    if (messageReceive) {
      if (selectChat._id == messageReceive.senderId) {
        ...
        setMessages([
          ...messages,
          { message: messageReceive.text, sender: messageReceive.senderId },
        ]);
        try {
          const token = localStorage.getItem("token");
        ...
        const obj = {
          sender: messageReceive.senderId,
          receiver: messageReceive.receiverId,
          message: messageReceive.text,
        };
  ...
  const selectChatHandler = (id) => {
    ...
    try {
      ...
      setMessageNotification(
        messageNotification.filter((message) => message.sender != id)
      );
    } catch (err) {
      console.log(err);
    }
      ...
      <div className={classes.chatbox}>
            ...
            {filteredUsers &&
              ...
              filteredUsers.map((alluser) => {
                if (alluser._id != auth.user.user._id) {
                  return (
                    <div
                      className={classes.leftbodylist}
                      onClick={() => {
        ...
        {isSelected && (
          <div className={classes.rightbox}>
            <div className={classes.rightheader}>
              <div className={classes.profile}>
                ...
                <div className={classes.status}>
                  ...
                  <p className={classes.stats}>
                    {onlineusers.find((user) => user.userId == selectChat._id)
                      ? "Online"
                      : "Offline"}
                  </p>
                </div>
              ...
              <div>
                {auth.user.user.blocklist.includes(selectChat._id) && (
                  <button className={classes.button} onClick={unblockHandler}>
                    Unblock
                  </button>
                )}
                {!auth.user.user.blocklist.includes(selectChat._id) && (
                  <button className={classes.button} onClick={blockHandler}>
                    Block
                  </button>
                )}
              </div>
              ...
              {messages.map((message, index) => {
                return (
                  <p
                    ref={el}
                    className={
                      message.sender == auth.user.user._id
                        ? classes.sent
                        : classes.received
                    }
                  >
        ...
        {!isSelected && (
          <div className={classes.rightbox}>
            ...
            >
              <img
                src="https://cdn-icons-png.flaticon.com/512/457/457975.png"
                style={{
                  height: "100%",
                  width: "100%",
                  marginLeft: "auto",
