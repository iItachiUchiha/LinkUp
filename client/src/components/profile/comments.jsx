...
const Comments = ({ comments, id, commentsState }) => {
  ...
  const deleteHandler = async (commentId) => {
    try {
      ...
      commentsState.setComment(
        commentsState.comment.filter((comment) => comment._id != commentId)
      );
      console.log(res);
    } catch (err) {
      console.log(err);
  ...
  const editHandler = async (e) => {
    ...
    try {
      ...
      console.log(res);
      commentsState.setComment(
        commentsState.comment.map((comment) => {
  ...
  return (
    <div className={classes.comments}>
      ...
      {Object(commentsState.comment).map((comment) => {
        return edit.id != comment._id ? (
          <div key={comment._id}>
            <div className={classes.profile}>
              <img
                src={comment.profilePic.url}
          ...
          <div key={comment._id}>
            ...
            <form className={classes.form2} onSubmit={editHandler}>
              <input
                ...
                value={edit.comment}
                onChange={(e) => {
                  setEdit({ comment: e.target.value, id: comment._id });
                }}
                className={classes.inputfield}
              ></input>
